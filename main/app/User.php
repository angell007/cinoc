...
class User extends Authenticatable
{
    ...
    {
        $cv = ProfileCv::where('user_id', '=', $this->id)
            ->where('is_default', '=', 1)
            ->first();
        if (null === $cv) {
    ...
    public function profileEducationNonFormal()
    {
        return $this->hasMany('App\Models\ProfileEducationNonFormal', 'user_id', 'id');
    }
    ...
    public function getProfileSkillsStr()
    {
        ...
        if ($profileSkills !== null) {
            foreach ($profileSkills as $profileSkill) {
                $jobSkill = JobSkill::where('job_skill_id', '=', $profileSkill->job_skill_id)
                    ->lang()
                    ->first();
    ...
    public function isFavouriteJob($job_slug)
    {
        $return = false;
        if (Auth::check()) {
            $count = FavouriteJob::where('user_id', Auth::user()->id)
                ->where('job_slug', 'like', $job_slug)
                ->count();
            if ($count > 0) {
                $return = true;
            }
    ...
    public function isAppliedOnJob($job_id)
    {
        $return = false;
        if (Auth::check()) {
            $count = JobApply::where('user_id', Auth::user()->id)
                ->where('job_id', '=', $job_id)
                ->count();
            if ($count > 0) {
                $return = true;
    ...
    public function isFavouriteCompany($company_slug)
    {
        $return = false;
        if (Auth::check()) {
            $count = FavouriteCompany::where('user_id', Auth::user()->id)
                ->where('company_slug', 'like', $company_slug)
                ->count();
            if ($count > 0) {
                $return = true;
            }
    ...
    public function getNameAttribute()
    {
        ...
        if (!empty($this->first_name)) {
            $html .= ucfirst(strtolower($this->first_name));
        }
        if (!empty($this->middle_name)) {
            $html .= ' ' . ucfirst(strtolower($this->middle_name));
        }
        if (!empty($this->first_lastname)) {
            $html .= ' ' . ucfirst(strtolower($this->first_lastname));
        }
        if (!empty($this->second_lastname)) {
            $html .= ' ' . ucfirst(strtolower($this->second_lastname));
        }
        return $html;
    }
    public function getName()
    {
        ...
        if (!empty($this->first_lastname)) {
            $html .= ' ' . $this->first_lastname;
        }
        if (!empty($this->second_lastname)) {
            $html .= ' ' . $this->second_lastname;
        }
        return $html;
    }
    ...
    public function getMaritalStatus($field = '')
    {
        ...
        if (null !== $maritalStatus) {
            if (!empty($field)) {
                return $maritalStatus->civil_status;
            } else {
    ...
    {
        return ApplicantMessage::where('user_id', '=', $this->id)
            ->where('is_read', '=', 0)
            ->count();
    }
