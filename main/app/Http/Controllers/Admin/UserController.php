...
class UserController extends Controller
{
    ...
    public function storeUser(UserFormRequest $request)
    {
        ...
        $user->car_license_category = $request->input('car_license_category');
        $user->motorcycle_license = $request->input('motorcycle_license');
        $user->motorcycle_license_category = $request->input('motorcycle_license_category');
        ...
        return Datatables::of($users)
            ...
            ->filter(function ($query) use ($request) {
                ...
                if ($request->has('date_of_birth') && !empty($request->date_of_birth)) {
                    ...
                    if ($request->date_of_birth == 'no_date') {
                        $query->whereNull('date_of_birth');
                    } else {
                        $query->where('users.date_of_birth', 'like', "%{$request->get('date_of_birth')}%");
                    }
                }
