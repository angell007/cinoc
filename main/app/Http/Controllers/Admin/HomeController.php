...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        $culminados = DB::table('participants')->where('status', 'Culminó')
        // ->whereMonth('created_at', Carbon::now()->month)
        ->whereMonth('updated_at', Carbon::now()->month)->count();
        $asistieron = DB::table('participants')->where('status', 'Asistió')
        // ->whereMonth('created_at', Carbon::now()->month)
        ->whereMonth('updated_at', Carbon::now()->month)->count();
        $direccionados = DB::table('participants')->where('status', 'Direccionado')
        // ->whereMonth('created_at', Carbon::now()->month)
        ->whereMonth('updated_at', Carbon::now()->month)->count();
