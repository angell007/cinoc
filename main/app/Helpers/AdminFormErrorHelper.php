    ...
    public static function showErrors($errors, $field)
    {
        ...
        if ($errors->first($field) != '') :
            foreach ($errors->get($field) as $message) :
                $html .= '<span id="name-error" class="help-block text-danger help-block-error">' . $message . '</span>';
            endforeach;
        endif;
        return $html;
    }
    ...
    public static function showErrorsNotice($errors)
    {
        ...
        if (count($errors) > 0) {
            ...
            foreach ($errors->all() as $message) :
                $html .= '<li><span id="name-error" class="help-block text-danger help-block-error">' . $message . '</span></li>';
            endforeach;
            $html .= '</ul></div>';
        }
        return $html;
