...
<div class="form-body">
    <div class="form-group">
        <button class="btn purple btn-outline sbold" onclick="showProfileExperienceModal();"> Nueva
            Experiencia
        </button>
    </div>
    <div class="row">
        <div class="col-md-12">
            <div class="portlet light portlet-fit bordered">
                <div class="portlet-title">
                    <div class="caption">
                        <i class=" icon-layers font-green">
                        </i>
                        <span class="caption-subject font-green bold uppercase">Experiencia
                        </span>
                    </div>
                </div>
...
@push('scripts')
    <script type="text/javascript">
            ...
            $.ajax({
                ...
                success: function(json) {
                    ...
                    $('.selected-remote').select2({
                        width: 'resolve',
                        dropdownParent: $('#add_experience_modal'),
        ...
        function showProfileExperienceEditModal(profile_experience_id, state_id = 0, city_id = 0) {
            $("#add_experience_modal").modal();
            loadProfileExperienceEditForm(profile_experience_id, state_id, city_id);
        }

        function loadProfileExperienceEditForm(profile_experience_id, state_id = 0, city_id = 0) {
            $.ajax({
                type: "POST",
        ...
        function delete_profile_experience(id) {
            if (confirm('Are you sure! you want to delete?')) {
                $.post("{{ route('delete.profile.experience') }}", {
                        id: id,
                        _method: 'DELETE',
                        _token: '{{ csrf_token() }}'
                    })
        ...
        $(document).ready(function() {
            showExperience();
            initdatepicker();
            $(document).on('change', '#experience_country_id',
                function(e) {
                    e.preventDefault();
                    filterDefaultStatesExperience(0, 0);
                });
            $(document).on('change', '#experience_state_id',
                function(e) {
                    e.preventDefault();
        ...
        function showExperience() {
            $.post("{{ route('show.profile.experience', $user->id) }}", {
                user_id: {{ $user->id }},
                _method: 'POST',
                _token: '{{ csrf_token() }}'
            }).done(function(response) {
