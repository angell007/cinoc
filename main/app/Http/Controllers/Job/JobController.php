...
class JobController extends Controller
{
    ...
    {
        $this->middleware('auth', ['except' => ['jobsBySearch', 'jobDetail', 'searchStudenByJobs', 'sendEmails']]);
    ...
    public function searchStudenByJobs($id)
    {
        
        $job = DB::table('jobs')->where('id', $id)->first();
        $users = User::select('users.id', 'users.search', 'users.rol', 'fa.functional_area', 'pcv.cv_file' )
          ->join('profile_cvs as pcv', 'pcv.user_id', 'users.id')
          ->leftJoin('functional_areas as fa', 'fa.id', 'users.functional_area_id')
          ->where('users.functional_area_id', $job->functional_area_id)
          ->where('pcv.cv_file', '<>', '')
          ->orWhereRaw("MATCH (`search`) AGAINST ('$job->title*' IN BOOLEAN MODE)")
        //   ->having('pcv.is_default', 1)
        ->get();
        return response()->json(  $users );
    }
    
    public function sendEmails($data, $company, $job)
    {
        
        
        $job = DB::table('jobs')->where('id', $job)->first();
        $company = DB::table('companies')->where('id', $company)->first();
        
         $users = User::select('users.id', 'users.first_lastname', 'users.first_name', 'pcv.title', 'pcv.cv_file' )
          ->join('profile_cvs as pcv', 'pcv.user_id', 'users.id')
        ...
        $email =  Mail::send('emails.candidatesinteresting',  [ 'users' => $users ,'job' => $job , 'company' => $company ], function($msj) use ($company, $job ) {
            ...
            $msj->to($company->email);
                ...
                foreach($users as $user){
                    ...
                    DB::table('send_emails')
                    ->insert(['user_id' =>  $user->id, 'company_id' => $company->id, 'job_id' => $job->id, 'send_date' => date('Y-m-d')]);
                }

                return response()->json( 'Emails enviado correctamente' );
    ...
    public function jobDetail(Request $request, $job_slug)
    {
        ...
        if (!(bool) $job->is_active) {
            ...
            if (!$companyUser || (int) $companyUser->id !== (int) $job->company_id) {
                abort(404);
            }
        }
    ...
    public function addToFavouriteJob(Request $request, $job_slug)
    {
        ...
        flash(__('Empleo agregado a la lista de favoritos'))->success();
    ...
    public function removeFromFavouriteJob(Request $request, $job_slug)
    {
        ...
        flash(__('Empelo eliminado de la lista de favoritos'))->success();
        return \Redirect::route('job.detail', $job_slug);
    }
    ...
    public function applyJob(Request $request, $job_slug)
    {
        ...
        if ((bool)$user->is_active === false) {
            flash(__('Por favor subir la documentación requerida. Sí ya lo hizo por favor comuníquese con nosotros'))->error();
            return \Redirect::route('job.detail', $job_slug);
            exit;
        }

        if (!ProfileCompletionHelper::canApplyToJobs($user)) {
            $completion = ProfileCompletionHelper::assess($user);
            flash('No puedes aplicar a esta vacante. Tu hoja de vida está al ' . $completion['percentage'] . '%. Debes completar al menos el ' . $completion['threshold'] . '% de los campos obligatorios. Pendientes: ' . implode(', ', $completion['missing']) . '.')->error();
    ...
    public function postApplyJob(ApplyJobFormRequest $request, $job_slug)
    {
        ...
        if (!ProfileCompletionHelper::canApplyToJobs($user)) {
            $completion = ProfileCompletionHelper::assess($user);
            flash('No puedes aplicar a esta vacante. Tu hoja de vida está al ' . $completion['percentage'] . '%. Debes completar al menos el ' . $completion['threshold'] . '% de los campos obligatorios.')->error();
            return \Redirect::route('my.profile');
        }
        ...
        flash(__('Has aplicado  a esta vacante '))->success();
        return \Redirect::route('job.detail', $job_slug);
    }
    ...
    public function myJobApplications(Request $request)
    {
      ...
      $my = Auth::user()->getAppliedJobApplyIdsArray();


      $jobs = Job::whereIn('job_apply.id', $my)->join('job_apply',"jobs.id",'=','job_apply.job_id')->get();
