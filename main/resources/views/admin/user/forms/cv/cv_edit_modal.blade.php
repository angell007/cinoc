    ...
    <div class="modal-content">
        <form class="form" id="add_edit_profile_cv" method="PUT" action="{{ route('update.profile.cv', [$profileCv->id, $user->id]) }}">
            ...
            <div class="modal-header">
                ...
                <h4 class="modal-title">Editar Cv</h4>
            </div>
            @include('admin.user.forms.cv.cv_form')
            <div class="modal-footer">
                <button type="button" class="btn dark btn-outline" data-dismiss="modal">Close</button>
                <button type="button" class="btn btn-large btn-primary" onClick="submitProfileCvForm();">Guardar Cv <i class="fa fa-arrow-circle-right" aria-hidden="true"></i></button>
            </div>
        </form>
    </div>
    <!-- /.modal-content --> 
