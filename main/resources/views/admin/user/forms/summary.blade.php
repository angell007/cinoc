...
<form class="form" id="add_edit_profile_summary" method="POST" action="{{ route('update.profile.summary', [$user->id]) }}">{{ csrf_field() }}
    <div class="form-body">
        ...
        <div class="form-group" id="div_summary">
            <label for="summary" class="bold">Resumen de perfil</label>
            <textarea name="summary" class="form-control" id="summary" placeholder="Resumen de perfil">{{ old('summary', (isset($user))? $user->getProfileSummary('summary'):'') }}</textarea>
            <span class="help-block text-danger summary-error"></span>
        </div>
        <button type="button" class="btn btn-large btn-primary" onClick="submitProfileSummaryForm();">Actualizar<i class="fa fa-arrow-circle-right" aria-hidden="true"></i></button>
    </div>
</form>
@push('scripts')
